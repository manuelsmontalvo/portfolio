...
function Card({ img, projectName, projectDescription, liveSite, github }) {
  return (
    <div className="my-4 flex flex-col w-min h-96 space-x-2 shadow-xl rounded-lg overflow-hidden mx-auto">
      <div className="card rounded-2xl p-6 flex flex-col min-w-sm border border-gray-700 bg-gray-700 text-gray-50 justify-center items-center  transition-shadow shadow-xl hover:shadow-xl min-w-max">
        <div className="card__media">
          <img src={img} className="rounded-2xl  h-48 w-96" alt="project" />
        </div>

        <div className="flex items-center p-4 max-w-sm min-h-25">
          <div className="relative flex flex-col items-center w-full justify-center  items-center">
            <div className="flex flex-col space-y-1 w-full">
              <span className="text-md whitespace-nowrap  text-gray-50 font-semibold">{projectName}</span>
              <p className="text-sm text-gray-200 max-h-full text-left md:break-words">{projectDescription}</p>
              <div className="py-2 flex space-x-4 my-6">
